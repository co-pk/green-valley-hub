...
const { sendApplicationEmails, updateApplicationStatus } = require('./utils/emailHandler');
...
// Admin Routes
app.get('/api/admin/pending-applications', authenticateAdmin, async (req, res) => {
  try {
...
app.post('/api/admin/decision', authenticateAdmin, async (req, res) => {
    ...
    try {
        ...
        const parentDoc = await db.collection('parents').doc(studentData.parentRef).get();
        if (!parentDoc.exists) {
            logger.error('Parent document not found', { studentId, parentRef: studentData.parentRef });
            return res.status(404).json({
                success: false,
...
app.post('/api/apply', async (req, res) => {
  ...
  const studentActivationCode = crypto.randomBytes(3).toString('hex').toUpperCase();
  const parentId = 'P' + crypto.randomBytes(3).toString('hex').toUpperCase();
  const parentActivationCode = crypto.randomBytes(3).toString('hex').toUpperCase();
  ...
  try {
    ...
    try {
      ...
      if (parentResponse && parentResponse.statusCode === 202) {
        ...
        throw new Error(`Parent email failed with status: ${parentResponse?.statusCode || 'unknown'}`);
      }
      ...
      if (studentResponse && studentResponse.statusCode === 202) {
        ...
        throw new Error(`Student email failed with status: ${studentResponse?.statusCode || 'unknown'}`);
      }
      ...
      console.error('Detailed error:', JSON.stringify(emailError.response?.body || emailError, null, 2));
      
      // Return a 500 status for email errors to clearly indicate the issue
      return res.status(500).json({ 
        message: 'Application was saved but there was an issue sending confirmation emails. Please contact support.',
    ...
    if (error.code === 'auth/invalid-credential') {
      return res.status(500).json({ message: 'Firebase authentication failed. Please check your configuration.' });
    }
    return res.status(500).json({ 
...
app.post('/api/auth/student/login', async (req, res) => {
  ...
  try {
    ...
    // Verify password
    const hashedPassword = crypto.pbkdf2Sync(password, student.salt, 1000, 64, 'sha512').toString('hex');
...
app.post('/api/auth/parent/login', async (req, res) => {
  ...
  try {
    ...
    const snapshot = await parentsRef.where('parentId', '==', parentId).limit(1).get();
    ...
    // Verify password
    const hashedPassword = crypto.pbkdf2Sync(password, parent.salt, 1000, 64, 'sha512').toString('hex');
    ...
    const studentsRef = db.collection('students');
    const childrenSnapshot = await studentsRef.where('parentId', '==', parentId).get();
...
app.post('/api/admin/approve-application', async (req, res) => {
  ...
  try {
    ...
    const studentsRef = db.collection('students');
    const studentSnapshot = await studentsRef.where('studentId', '==', studentId).limit(1).get();
    ...
    const parentsRef = db.collection('parents');
    const parentSnapshot = await parentsRef.where('parentId', '==', parentId).limit(1).get();
...
app.post('/api/activate', async (req, res) => {
  ...
  try {
    const studentsRef = db.collection('students');
    const snapshot = await studentsRef.where('studentEmail', '==', email).limit(1).get();
...
app.get('/api/checkActivation/:studentId', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ message: 'Failed to check activation status' });
  }
});
